...
function setValueTextbox(v,boxId,txt){
	acSearchField.val(v + ': ' + txt);	
	acValue.val(v);

	// Under Code is not use
...
function setAutoComplete(field_id, results_id, get_url){
	// initialize vars
	acSearchId  = "#" + field_id[1];
	acResultsId = "#" + results_id;
	acURL 		= get_url;
	// create the results div
	$('.ac-item').remove();
	$("body").append('<div class="ac-item" id="' + results_id + '"></div>');
	...
	$.getJSON(acURL + part, function(json){
		...
		if(ansLength > 0){
			...
			for(i=0; i < ansLength; i++) {
				...
				newData += '<div class="unselected" onclick="setValueTextbox(\''+values[0]+'\',\''+values[2]+'\', \''+values[3]+'\');">' + values[0] + '<br/>&nbsp;&nbsp;&nbsp;' + values[2] + '<br/>&nbsp;&nbsp;&nbsp;' + values[3] + '<br/>&nbsp;&nbsp;&nbsp;' + values[4] +'</div>';
			}
