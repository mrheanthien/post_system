				...
				if ($.isArray(items)) {
					...
					$.each(items, function (i, opt) {
						if (typeof valuesHash[opt.id] === 'undefined') {
							$('<option value="' + opt.id + '" title="' + opt.full_text + '">' + opt.id + '. ' + opt.text + '</option>').appendTo(select);
						}
					});
				} else {
					...
					$.each(items, function (value, text) {
						if (typeof valuesHash[value] === 'undefined') {
							$('<option value="' + value + '" title="' + opt.full_text + '">' + value + '. ' + text + '</option>').appendTo(select);
						}
					});
				}
			}
    ...
    function processValue(e) {
        ...
        (e.which && (
			...
			if ($.isArray(ajaxOptions.data)) {
				...
				$.extend(ajaxOptions.data, { data: {q: q, l:ajaxOptions.l} });
			}
